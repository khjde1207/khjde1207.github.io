...
1. Kuinka tarkastella tiedostoja:
  - Napsauta Lisää tiedostoa asiakirjassani lisätäksesi TXT: tä.
  
  - Kosketa TXT -tiedostoa ja painamalla alaosassa olevaa painiketta nähdäksesi tiedoston.


2. Kuinka piilottaa katsoja -mainokset / pohja navigointipainikkeet:
  - Kaksinkertainen napauta näytön keskustaa.
...
3. Painikkeen Mitt -toimintoalueen muuttaminen:
  ...
  - Voit asettaa useita toimintoja, mukaan lukien koko näytön, sivun liikkeen, kirkkauden säätö ja äänenvoimakkuuden säätö.
...
4. Fontin tai kirjasimen koon muuttaminen:
  ...
  - Voit käyttää kirjasinta painamalla latauspainiketta saadaksesi kymmeniä ilmaisia ​​fontteja.


5. Kuvan tekstin muuntamisen käyttö:
  - Voit purkaa tekstiä pakatusta kuvasta.
  
  - Tätä kutsutaan OCR -toiminnoksi.
  ...
  - OCR toimii AI: n kanssa, joten nopeus ja tarkkuus ovat melko hyviä.
...
6. EPUB, PDF -muuntamismenetelmä:
  ...
  - Sitä käytetään pääasiassa kirjeiden rikkoutumiseen ja niiden näkyviin vain tietyissä kirjasimissa.
...
7. Kuinka asentaa TTS:
  - TTS: ään liittyviä asetuksia on keskittynyt asetuksiin - TTS -asetukset, TTS -suodatinasetukset.
  
  - TTS -asetukset sisältävät yksinkertaisia ​​TTS -ominaisuuksia, kuten ääni, nopeus, äänenvoimakkuus, painikkeen läpinäkyvyys, lauseet päättävät merkit ja kerrallaan luevien lauseiden lukumäärä sekä TTS -ominaisuuksia täydentävät laitteet.
  
  - Kerralla luevien lauseiden lukumäärä on lisätty toiminto, jonka avulla useita rivejä voidaan lukea kerralla, jotta ne voidaan kuulla pienellä viiveellä.
...
8. Varmuuskopiointi- ja palautusominaisuuksien kuvaus:
  - Asennus - Varmuuskopiointi ja palautus on ominaisuus, jonka avulla useita laitteita voi käyttää sovellusta samassa ympäristössä.
...
9. Kuinka käyttää tekstin erämuutoksia:
  -1. [asetettu-set-show Muokkaa-painike]
  
  - 2. [Asetukset - irtotavarana tekstin muokkaaminen]
  
  - Voit käyttää erän muokkausta kahdella tavalla.
  ...
  - Voit myös ladata ja käyttää usein esiintyviä tunnistusvirheitä tekstin erän korjaamiseen.
...
11. Sivun liittyvä kuvaus:
  - Jos käytät erityisiä toimintoja, kuten TTS -lauseen tarkastus, erän korjaus, linjojen uudelleenorganisointi ja käännös, sivuja kulutetaan, mutta koska se on noin 10-50, et tunne sitä kuluttavan.

  - Suurinta osaa sivuja käytetään todennäköisesti tuntemaan tekstin ottaessa OCR: stä, EPUB: sta ja PDF: stä.

  - Tekstiä otettaessa emme vain poimia yksin, vaan myös järjestämme lauseita tekemällä kymmeniä ongelmia, jotka ilmenevät poiminnan jälkeen kuvioihin.

  - Se oli todella vaikea toteuttaa, joten pyydän ymmärrystäsi.
...
12. Mainonnan kuvaus:
  ...
  - Mainoksia poistettaessa voit käyttää sivua mainoksien poistamiseen tietyn ajanjakson ajan. (Lukuun ottamatta uniaikaa, se on noin päivä.)

  -Niille, jotka ovat pinottavia sivuja, voit poistaa katsojapuolen mainoksen kuluttamalla 2000 sivua.
  ...
  - 8000 sivua on lisätty niille, joita häiritsevät lyhyet mainokset, jotka ilmestyvät suuren määrän muuntamisen aikana.
...
13. Ominaisuuden kuvaus EAR: lle (historian interworing):
  ...
  - Jos sama asiakirja pysyy, epätarkkoja tietoja voidaan näyttää.
...
14. TTS -lauseen tarkastaja Kuvaus:
  ...
  - Järjestäin tämän kuvion ja tuotin sen painottamalla kutakin kuviota.
...
16. Käännöstoiminto Kuvaus:
  ...
  - Verkkoselaimen käännös on toiminto, joka näyttää lauseet käännettäväksi selaimessa.

  - Jos käynnistät käännöstoiminnon asetuksissa - käännösasetukset ja painaa lausetta, käännöstoiminto toimii.


17. Muut asiat:
  - Kun avaat asiakirjan, merkintä, joka toistuu yli kolme kertaa, avataan vaihtamalla kaksi kertaa.
